...
public fun addViewsAtAnchorEdge(
    ...
    return layoutManager.convertAdapterDirToMovementDir(CustomLayoutManager.TOWARDS_LOWER_INDICES)
}
...
public fun addViewsAtOptAnchorEdge(
    ...
    return layoutManager.convertAdapterDirToMovementDir(CustomLayoutManager.TOWARDS_HIGHER_INDICES)
}
...
public fun estimateShortestRoute(
    ...
    if (layoutManager.topLeftIndex == adapterIndex) {
        return CustomLayoutManager.TOWARDS_TOP_LEFT
    } else if (layoutManager.bottomRightIndex == adapterIndex) {
        return CustomLayoutManager.TOWARDS_BOTTOM_RIGHT
    }
    ...
    val adapterDir = when {
        targetIsSmaller && minDistIsInLoop -> CustomLayoutManager.TOWARDS_LOWER_INDICES
        targetIsSmaller && minDistIsOverSeam -> CustomLayoutManager.TOWARDS_HIGHER_INDICES
        targetIsLarger && minDistIsInLoop -> CustomLayoutManager.TOWARDS_HIGHER_INDICES
        targetIsLarger && minDistIsOverSeam -> CustomLayoutManager.TOWARDS_LOWER_INDICES
        else -> throw IllegalStateException()  // Should never happen.
    }
